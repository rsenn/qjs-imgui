...
/*
thread_local VISIBLE JSClassID js_imgui_class_id = 0;
thread_local JSValue imgui_proto = {JS_TAG_UNDEFINED}, imgui_ctor = {JS_TAG_UNDEFINED};

thread_local VISIBLE JSClassID js_imguientry_class_id = 0;
thread_local JSValue imguientry_proto = {JS_TAG_UNDEFINED}, imguientry_ctor = {JS_TAG_UNDEFINED};

enum { IMGUI_METHOD_READ, IMGUI_METHOD_WRITE, IMGUI_METHOD_READFILE, IMGUI_METHOD_WRITEFILE };
enum { IMGUI_PROP_FORMAT, IMGUI_PROP_COMPRESSION, IMGUI_PROP_FILTERS, IMGUI_PROP_FILECOUNT };

enum {
  IMGUIENTRY_METHOD_READ,
  IMGUIENTRY_METHOD_WRITE,
  IMGUIENTRY_METHOD_READFILE,
  IMGUIENTRY_METHOD_WRITEFILE
};
enum {
  ENTRY_ATIME,
  ENTRY_BIRTHTIME,
...
static JSValue js_imguientry_wrap_proto(JSContext* ctx, JSValueConst proto, struct imgui_entry* ent);
static JSValue js_imguientry_wrap(JSContext* ctx, struct imgui_entry* ent);

struct ImGuiInstance {
  JSValue imgui;
};
struct ImGuiEntryRef {
  JSContext* ctx;
  JSValueConst callback, args[2];
};

static void
js_imgui_free_buffer(JSRuntime* rt, void* opaque, void* ptr) {
  struct ImGuiInstance* ainst = opaque;
  JS_FreeValueRT(rt, ainst->imgui);
  js_free_rt(rt, ainst);
}

static void
js_imgui_progress_callback(void* opaque) {
  struct ImGuiEntryRef* aeref = opaque;
...
struct imgui*
js_imgui_data(JSContext* ctx, JSValueConst value) {
  struct imgui* ar;
  ar = JS_GetOpaque2(ctx, value, js_imgui_class_id);
  return ar;
}

static JSValue
js_imgui_wrap_proto(JSContext* ctx, JSValueConst proto, struct imgui* ar) {
  JSValue obj;

  if(js_imgui_class_id == 0)
    js_imgui_init(ctx, 0);
  if(JS_IsNull(proto) || JS_IsUndefined(proto))
    proto = JS_DupValue(ctx, imgui_proto);

  // using new_target to get the prototype is necessary when the class is extended.
  obj = JS_NewObjectProtoClass(ctx, proto, js_imgui_class_id);
...
static JSValue
js_imgui_wrap(JSContext* ctx, struct imgui* ar) {
  return js_imgui_wrap_proto(ctx, imgui_proto, ar);
}
*/
static JSValue
...
JS_CFUNC_MAGIC_DEF("SetWindowPos", 2, js_imgui_functions, IMGUI_SET_WINDOW_POS),
JS_CFUNC_MAGIC_DEF("SetWindowSize", 2, js_imgui_functions, IMGUI_SET_WINDOW_SIZE),
JS_CFUNC_MAGIC_DEF("SetWindowCollapsed", 2, js_imgui_functions, IMGUI_SET_WINDOW_COLLAPSED),
JS_CFUNC_MAGIC_DEF("SetWindowFocus", 0, js_imgui_functions, IMGUI_SET_WINDOW_FOCUS),
...
JS_CFUNC_MAGIC_DEF("GetColorU32", 2, js_imgui_functions, IMGUI_GET_COLORU32),
...
JS_CFUNC_MAGIC_DEF("PushID", 2, js_imgui_functions, IMGUI_PUSH_ID),
JS_CFUNC_MAGIC_DEF("PushID", 1, js_imgui_functions, IMGUI_PUSH_ID),
JS_CFUNC_MAGIC_DEF("PushID", 1, js_imgui_functions, IMGUI_PUSH_ID),
JS_CFUNC_MAGIC_DEF("PopID", 0, js_imgui_functions, IMGUI_POP_ID),
JS_CFUNC_MAGIC_DEF("GetID", 1, js_imgui_functions, IMGUI_GET_ID),
JS_CFUNC_MAGIC_DEF("GetID", 2, js_imgui_functions, IMGUI_GET_ID),
JS_CFUNC_MAGIC_DEF("GetID", 1, js_imgui_functions, IMGUI_GET_ID),
...
JS_CFUNC_MAGIC_DEF("RadioButton", 2, js_imgui_functions, IMGUI_RADIO_BUTTON),
...
JS_CFUNC_MAGIC_DEF("Combo", 5, js_imgui_functions, IMGUI_COMBO),
JS_CFUNC_MAGIC_DEF("Combo", 4, js_imgui_functions, IMGUI_COMBO),
...
JS_CFUNC_MAGIC_DEF("TreeNode", 1, js_imgui_functions, IMGUI_TREE_NODE),
...
JS_CFUNC_MAGIC_DEF("TreeNodeV", 3, js_imgui_functions, IMGUI_TREE_NODE_V),
JS_CFUNC_MAGIC_DEF("TreeNodeEx", 2, js_imgui_functions, IMGUI_TREE_NODE_EX),
...
JS_CFUNC_MAGIC_DEF("CollapsingHeader", 2, js_imgui_functions, IMGUI_COLLAPSING_HEADER),
...
JS_CFUNC_MAGIC_DEF("Selectable", 4, js_imgui_functions, IMGUI_SELECTABLE),
JS_CFUNC_MAGIC_DEF("ListBox", 5, js_imgui_functions, IMGUI_LIST_BOX),
JS_CFUNC_MAGIC_DEF("ListBox", 6, js_imgui_functions, IMGUI_LIST_BOX),
JS_CFUNC_MAGIC_DEF("ListBoxHeader", 2, js_imgui_functions, IMGUI_LIST_BOX_HEADER),
...
JS_CFUNC_MAGIC_DEF("PlotHistogram", 9, js_imgui_functions, IMGUI_PLOT_HISTOGRAM),
JS_CFUNC_MAGIC_DEF("Value", 2, js_imgui_functions, IMGUI_VALUE),
JS_CFUNC_MAGIC_DEF("Value", 2, js_imgui_functions, IMGUI_VALUE),
JS_CFUNC_MAGIC_DEF("Value", 2, js_imgui_functions, IMGUI_VALUE),
...
JS_CFUNC_MAGIC_DEF("IsRectVisible", 1, js_imgui_functions, IMGUI_IS_RECT_VISIBLE),
...
js_imgui_init(JSContext* ctx, JSModuleDef* m) {
 ...
 /* if(js_imgui_class_id == 0) {
    JS_NewClassID(&js_imgui_class_id);
    JS_NewClass(JS_GetRuntime(ctx), js_imgui_class_id, &js_imgui_class);

    imgui_ctor = JS_NewCFunction2(ctx, js_imgui_constructor, "ImGui", 1, JS_CFUNC_constructor, 0);
    imgui_proto = JS_NewObject(ctx);

    JS_SetPropertyFunctionList(ctx, imgui_proto, js_imgui_funcs, countof(js_imgui_funcs));
    JS_SetPropertyFunctionList(ctx, imgui_ctor, js_imgui_static_funcs, countof(js_imgui_static_funcs));
    JS_SetClassProto(ctx, js_imgui_class_id, imgui_proto);

    JS_NewClassID(&js_imguientry_class_id);
    JS_NewClass(JS_GetRuntime(ctx), js_imguientry_class_id, &js_imguientry_class);

    imguientry_ctor = JS_NewCFunction2(ctx, js_imguientry_constructor, "ImGuiEntry", 1, JS_CFUNC_constructor, 0);
    imguientry_proto = JS_NewObject(ctx);

    JS_SetPropertyFunctionList(ctx, imguientry_proto, js_imguientry_funcs, countof(js_imguientry_funcs));
    JS_SetClassProto(ctx, js_imguientry_class_id, imguientry_proto);
  }*/

  if(m) {
    JS_SetModuleExportList(ctx, m, js_imgui_static_funcs, countof(js_imgui_static_funcs));
   /* JS_SetModuleExport(ctx, m, "ImGui", imgui_ctor);
    JS_SetModuleExport(ctx, m, "ImGuiEntry", imguientry_ctor);*/
  }
...
JS_INIT_MODULE(JSContext* ctx, const char* module_name) {
  ...
  if(!(m = JS_NewCModule(ctx, module_name, &js_imgui_init)))
    return m;
  JS_AddModuleExport(ctx, m, "ImGui");
  return m;
}
