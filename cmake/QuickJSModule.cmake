...
function(write_module_cmake FNAME DEPS LIBRARIES CFLAGS)
  string(REGEX REPLACE "_" "-" NAME "${FNAME}")
  string(REGEX REPLACE "-" "_" VNAME "${FNAME}")

  # list(REMOVE_DUPLICATES) errors on an empty list, so guard it
  set(UNIQUE_LIBRARIES "${LIBRARIES}")
  list(REMOVE_ITEM UNIQUE_LIBRARIES "")
  if(UNIQUE_LIBRARIES)
    list(REMOVE_DUPLICATES UNIQUE_LIBRARIES)
  endif(UNIQUE_LIBRARIES)

  set(OUT "# Generated by make_module(${FNAME}) - describes quickjs-${VNAME}.a's own build\n")
  set(OUT
      "${OUT}set(${VNAME}_STATIC_LIBRARY \"${CMAKE_CURRENT_BINARY_DIR}/quickjs-${VNAME}.a\")\n")
  set(OUT "${OUT}set(${VNAME}_STATIC_LIBRARIES \"${UNIQUE_LIBRARIES}\")\n")
  set(OUT "${OUT}set(${VNAME}_STATIC_DEPENDENCIES \"${DEPS}\")\n")
  set(OUT "${OUT}set(${VNAME}_STATIC_CFLAGS \"${CFLAGS}\")\n")

  file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/quickjs-${VNAME}.cmake" "${OUT}")
endfunction(write_module_cmake FNAME DEPS LIBRARIES CFLAGS)
...
function(make_module FNAME)
  ...
  if(BUILD_STATIC_MODULE)
    ...
    # -fPIC for the SHARED target above, so the two can't share object files.
    add_library(${STATIC_TARGET_NAME} STATIC ${SOURCES})

    set_target_properties(
      ${STATIC_TARGET_NAME}
      PROPERTIES PREFIX "" OUTPUT_NAME "quickjs-${VNAME}" SUFFIX ".a"
                 ARCHIVE_OUTPUT_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}"
                 COMPILE_FLAGS "${MODULE_COMPILE_FLAGS}" POSITION_INDEPENDENT_CODE OFF)

    target_compile_definitions(
      ${STATIC_TARGET_NAME} PRIVATE _GNU_SOURCE=1 JS_${UNAME}_MODULE=1
                            QUICKJS_PREFIX="${QUICKJS_INSTALL_PREFIX}")

    target_link_libraries(${STATIC_TARGET_NAME} PUBLIC ${LIBS} ${QUICKJS_LIBRARY})

    config_module(${STATIC_TARGET_NAME})

    set(LIBRARIES ${${VNAME}_LIBRARIES})
    if(LIBRARIES)
      target_link_libraries(${STATIC_TARGET_NAME} PRIVATE ${LIBRARIES})
    endif(LIBRARIES)
    if(DEPS)
      add_dependencies(${STATIC_TARGET_NAME} ${DEPS})
    endif(DEPS)
